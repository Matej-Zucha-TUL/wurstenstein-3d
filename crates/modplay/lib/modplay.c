...
ModPlayerStatus_t *ProcessMOD() {
	if(mp.tick == 0) {
		if(mp.rules != NULL) {
			const ModRule_t *const *rules = mp.rules;

			while(*rules != NULL) {
				const ModRule_t *rule = *rules++;

				if(rule->order >= 0 && rule->order != mp.order) continue;
				if(rule->row >= 0 && rule->row != mp.row) continue;

				const ModAction_t *const *actions = rule->actions;

				while(*actions != NULL) {
					const ModAction_t *action = *actions++;

					switch(action->kind) {
						case Jump:
							mp.row = action->jump.row;
							mp.order = action->jump.order;
							break;
...
ModPlayerStatus_t *InitMOD(const uint8_t *mod, uint32_t samplerate) {
	...
	if(((signature & 0xFFFFFF00) == 0x464C5400) && ((signature & 0xFF) >= '1') && ((signature & 0xFF) <= '9')) {
		channels = (signature & 0xFF) - '0';
	}

	// xCHN

	if(((signature & 0x00FFFFFF) == 0x0043484E) && ((signature >> 24) >= '1') && ((signature >> 24) <= '9')) {
	...
	if((signature & 0x0000FFFF) == 0x00004348) {
		int high = ((signature >> 24) & 0xFF);
		int low = ((signature >> 16) & 0xFF);

		if(high >= '0' && high <= '9' && low >= '0' && high <= '9') {
			channels = (high - '0') * 10 + (low - '0');
		}
	}

	if(channels < 2 || channels >= CHANNELS) {
		return NULL;
	}
...
ModPlayerStatus_t *UpdateMODRules(const ModRule_t *const *rules) {
	mp.rules = rules;
