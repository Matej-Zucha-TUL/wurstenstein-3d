...
fn main() {
	...
	unsafe {
		...
		let _ = event_loop.run(move |event, event_loop| {
			...
			if egui.is_none() {
				egui = Some(egui_glow::EguiGlow::new(
					event_loop,
					gl.clone(),
					None,
					None,
			...
			} = event
			{
				...
				match event {
					...
					WindowEvent::RedrawRequested => {
						...
						camera.set_target(glm::vec3(0.0, 0.0, -10.0));
						...
						egui.as_mut().unwrap().run(&window, |ctx| {
							egui::Window::new("Wokýnko")
								...
								.show(ctx, |ui| {
									...
									let [x, y, z] =
										camera.get_position().as_slice().try_into().unwrap();
									...
									ui.horizontal(|ui| {
										ui.color_edit_button_rgb(
											(&mut background_color[..3]).try_into().unwrap(),
										);
										ui.label("Background");
									});
						...
						if last_update.elapsed().unwrap().as_secs_f32() >= fps_update_interval_secs
						{
							...
							let cursor_lock_string =
								format!("Cursor lock = {}", if cursor_lock { "on" } else { "off" });
							window.set_title(&format!(
								"Triangle - {}, {}, {}",
								fps_string, vsync_string, cursor_lock_string
							));
						...
						window.set_fullscreen(
							fullscreen.then_some(Fullscreen::Borderless(window.current_monitor())),
						);
